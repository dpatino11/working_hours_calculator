...
class Process:
    ...
    def calulate_payment(configuration:Options , workdays_hours:list) -> str:
        """_summary_
        ...
        for day in workdays_hours:
            try:
                ...
                day_week = configuration.get_shcedule(configuration, day_name)
                daypayment = 0
                for i in range(int(hours_delta)):
                    daypayment += configuration.get_payment(configuration, day_week, start_time.hour + i)
                day_payments.append({
                    'day_name': day_name
